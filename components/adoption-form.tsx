...
export default function AdoptionForm({ petId }: AdoptionFormProps) {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    formData.append("Hayvan ID", petId);
  ...
  if (isSubmitted) {
    return (
      <div className="max-w-2xl w-full p-12 text-center animate-in fade-in zoom-in duration-500 relative bg-card rounded-[2.5rem] border border-border/50 shadow-2xl mx-auto my-12">
        <button
          type="button"
          onClick={() => router.push("/")}
      ...
      <CardContent className="p-8">
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <Input id={birthDateId} name="Dogum Tarihi" type="date" required />
          </div>
